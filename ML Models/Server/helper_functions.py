...
def make_predictions(test_df, models):
    ...
    if isinstance(anomaly_scores, np.ndarray) and len(anomaly_scores) == 1:
        anomaly_scores = anomaly_scores[0].item()
        predicted_anomalies = predicted_anomalies[0].item()
    ...
    if isinstance(predicted_health_scores, np.ndarray) and len(predicted_health_scores) == 1:
        predicted_health_score = predicted_health_scores[0].item()
    else:
    ...
    if len(X_maint_scaled) == 1:
        ...
        predicted_days_to_maintenance = y_pred_maint[0][0].item()
    else:
        time_steps = 48
        if len(X_maint_scaled) >= time_steps:
            ...
            for i in range(len(y_pred_maint)):
                ...
                if idx < len(results_df):
                    results_df.iloc[idx, results_df.columns.get_loc('predicted_days_to_maintenance')] = y_pred_maint[i][0].item()
        else:
